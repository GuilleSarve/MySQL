...
select nombre,cargo_emp from personal.empleados order by sal_emp;
...
select sal_emp,comision_emp from personal.empleados where id_depto = 2000 order by comision_emp;
...
select nombre,sum(sal_emp + comision_emp+500) as total from personal.empleados where id_depto = 3000 group by id_emp;
...
select nombre,sal_emp,comision_emp,sum(sal_emp + comision_emp) as "Salario Total" from personal.empleados where comision_emp > 1000;
-- 14)Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen comisión.
select nombre,sal_emp,comision_emp,sum(sal_emp + comision_emp) as "Salario Total" from personal.empleados where comision_emp = 0;
...
select nombre_depto from personal.departamentos where nombre_depto = "Ventas" or nombre_depto = "Investigacion" or nombre_depto = "Mantenimiento";
-- 19)Ahora obtener los nombres de los departamentos que no sean “Ventas” ni “Investigación” ni ‘Mantenimiento.
select nombre_depto from personal.departamentos where nombre_depto != "Ventas" and nombre_depto != "Investigacion" and nombre_depto != "Mantenimiento";
...
select id_depto, avg(sal_emp) as Promedio from personal.empleados group by id_depto;
-- Consultas Having
...
-- 25)Mostrar el código, junto al número de empleados que dirige. Solo los que tengan más de dos empleados (2 incluido).
select cod_jefe, count(id_emp) from personal.empleados group by cod_jefe having count(id_emp)>=2;
-- 26)Hallar los departamentos que no tienen empleados
insert into departamentos(id_depto,nombre_depto,ciudad,cod_director) values (4400,"MANTENIMIENTO","ROMA",16759060);
	select id_depto from personal.departamentos where id_depto not in (select id_depto from personal.empleados group by id_depto);
	select departamentos.id_depto from personal.departamentos left join personal.empleados on departamentos.id_depto = empleados.id_depto group by id_depto having count(id_emp) = 0;
...
select * from personal.empleados where sal_emp>=(select avg(sal_emp) from personal.empleados);